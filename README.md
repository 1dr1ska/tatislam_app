...
- **Разделы** — управляемые из админки категории (имя, slug, видимость,
  порядок, фоновое изображение). Можно назначить раздел по умолчанию для
  фотопубликаций — он будет автоматически подставляться основным при создании
  новой фото-публикации. Фон раздела формирует дизайн главного экрана
