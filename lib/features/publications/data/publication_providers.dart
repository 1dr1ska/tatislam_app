...
final publicationRemoteDataSourceProvider =
    Provider<PublicationRemoteDataSource>((ref) {
...
final getPublicationsProvider =
    Provider<Future<List<Publication>> Function({bool includeAllStatuses})>((
      ref,
    ) {
      final repository = ref.watch(publicationRepositoryProvider);
      return ({bool includeAllStatuses = false}) =>
          repository.getPublications(includeAllStatuses: includeAllStatuses);
    });

/// Provider for getting publications with filters
final getPublicationsWithFiltersProvider =
    Provider<
      Future<List<Publication>> Function({
        String? sectionId,
        String? searchQuery,
        String? type,
        int limit,
...
final searchPublicationsProvider =
    Provider<
      Future<List<Publication>> Function(
        String query, {
        bool includeAllStatuses,
    ...
    >((ref) {
      final repository = ref.watch(publicationRepositoryProvider);
      return (query, {bool includeAllStatuses = false}) =>
          repository.getPublications(
            searchQuery: query,
            includeAllStatuses: includeAllStatuses,
          );
...
final getPublicationsByIdsProvider =
    Provider<Future<List<Publication>> Function(List<String> ids)>((ref) {
      final repository = ref.watch(publicationRepositoryProvider);
